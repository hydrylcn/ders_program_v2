...
def resource_path(relative_path):
    """ EXE içinde ve dışında dosya yollarını doğru bulmayı sağlar. """
...
class Scheduler:
    def __init__(self, assignments, classrooms, preferences=[], constraints={}, ayarlar=None):
        ...
        # --- DİNAMİK SINIF LİMİTİ HESAPLAMA (GÜNCELLENDİ) ---
        self.class_limits = {}
        counts = {}
        ...
        for cls, count in counts.items():
            ...
            # Kısıt listesindeki herhangi bir kural bu sınıfı kapsıyor mu?
            # Inverse (!) olsa bile is_match doğru çalışarak kapasiteyi esnetir.
            is_affected_by_constraint = False
            for keyword, rules in self.SPECIAL_CONSTRAINTS.items():
                # Ters kısıt (!) veya normal kısıt fark etmeksizin eşleşme kontrolü
            ...
            if is_affected_by_constraint:
                # Kısıtlanan grupların aynı anda birden fazla ders yapabilmesine izin ver
                current_limit = 10
            elif count > (len(self.DAYS) * len(self.SLOTS)):
                # Doğal çakışma zorunluluğu varsa limiti 2 yap
                current_limit = 2
    ...
    def is_valid(self, assignment, day, slot, classroom_name):
        ...
        # 1. Özel Kısıt Kontrolü (Arayüzdeki X butonu burayı etkiler)
        for keyword, rules in self.SPECIAL_CONSTRAINTS.items():
            ...
            for s in slots:
                if d in pref_df.index and s in pref_df.columns:
                    ...
                    if pd.notna(cell) and str(cell).strip() != "":
                        entries = [e.strip() for e in str(cell).replace(',', '\n').split('\n') if e.strip()]
...
def report_final(schedule, score, max_days):
    ...
    if not fazla_gun.empty:
        print(f"{max_days} günden fazla gelen hocalar:")
        for hoca, gun_sayisi in fazla_gun.items():
            print(f"   [!] {hoca}: {gun_sayisi} gün")
    else:
...
def arayuzden_baslat(ayarlar):
    try:
        ...
        total_trials = ayarlar["MAX_TRIALS"]
        for trial in range(1, total_trials + 1):
            ...
            if s.backtrack():
                ...
                print(f"Deneme {trial}/{total_trials} başarılı. (Skor: {cur_score})")
            else:
                print(f"Deneme {trial}/{total_trials} başarısız.")
        if best_schedule:
            ...
            print(f"BAŞARILI! Sonuç: {os.path.basename(ayarlar['OUTPUT_FILE'])} (Skor: {best_score})")
            return True
        return False
